...
tpu-inference's native flax/nnx path serves it via `KerasNNXModel`.
"""
...
def _export_sentencepiece(tokenizer, temp_dir: str, proto: bytes) -> bool:
    ...
    try:
        if _verify_tokenizer_dir(temp_dir):
            return True
        raise ValueError(
            "exported SentencePiece tokenizer produced empty output"
        )
    except Exception as e:  # noqa: BLE001
        logging.warning(
            "SentencePiece tokenizer unusable (%s); skip_tokenizer_init=True "
...
def _copy_preset_tokenizer_json(preset: str, temp_dir: str) -> bool:
    ...
    try:
        ...
        if len(reloaded("hello world").get("input_ids", [])) > 0:
            return True
        logging.warning(
            "Preset tokenizer.json for %r produced empty output.", preset
        )
    except Exception as e:  # noqa: BLE001
...
def setup_vllm_model(preset: str, dtype: str = "float16") -> str:
    ...
    Writes a standard architecture name (so vLLM's arch validation passes) plus
    ``keras_hub_preset``; tpu-inference's native loader routes that to
    ``KerasNNXModel`` on the flax/nnx path.
    ...
    # it for KV-cache profiling; the gemma default mis-sizes GPT-2/OPT).
    if vocab_size is None:
        if "gpt2" in preset_lower or "gpt_2" in preset_lower:
            ...
            vocab_size = 256000
        logging.warning(
            "vocab_size not inferred for %r; defaulting to %d.",
            preset,
            vocab_size,
        )

    # A vLLM-known architecture name so arch validation passes; tpu-inference
    # routes to KerasNNXModel by the `keras_hub_preset` key below (not by name).
    _std_arch = {
    ...
    architecture = _std_arch.get(model_type, "GPT2LMHeadModel")
    ...
    class KerasHubLLM(_BaseLLM):
        ...
        def __init__(self, model, **kwargs):
            if isinstance(model, str) and model.startswith("keras_hub:"):
                ...
                # Native flax/nnx path: don't force torchax. tpu-inference's
                # loader routes to KerasNNXModel by keras_hub_preset.
