...
def test_returns_none_when_context_inactive():
    vllm_context.clear_vllm_context()
    z = ops.zeros((1, 2, 4, 8))
    assert vllm_paged_attention(z, z, z, None, 0.125) is None
...
def test_shape_conversion_and_argument_order():
    ...
    out, new_cache = vllm_paged_attention(q, k, v, cache, scale=0.125)
...
def test_num_kv_heads_inferred_for_gqa():
    ...
    vllm_paged_attention(q, k, v, ops.zeros((1, 1)), scale=0.5)
